...
export default function Detail() {
  ...
  const fetchPlanet = async () => {
    ...
    if (a.user_planets?.length) {
      ...
      if (userPlanet?.level) {
        const allBuildsLength = Number(window.user.wallet?.builds?.length ?? 0);
  ...
  const getInitState = (a) => {
    setValue(
      window?.user?.wallet?.value?.find((bal) => bal.element === a?.element?.id)
        ?.value
    );
  };
  ...
  const userHasPlanet = () => {
    if (planet?.user_planets) {
      ...
      const idx = plData?.find(
        (item) =>
          item?.planetId === planet?.id && item?.userId === window?.user?.id
      );
      if (idx?.id) {
        return true;
      }
    ...
    if (window?.user?.userPlanets?.length) {
      ...
      if (planets.some((item) => item.planetId === planet?.id)) {
        const planeta = planets.find((item) => item.planetId === planet?.id);
          ...
          <div className="planet__detail">
            ...
            >
              ...
              <div className="planet__detail-header container without max-visible m-hidden">
                <div className="header-planet">
                  <span>
                    {planet?.name}({planet?.element?.symbol}) - Planet #
            ...
            <div className="planet__detail-wrapper container without">
              ...
              <div className="planet-information">
                <span className="info-header">{t("information")}</span>
              ...
              <div className="planet-farm max-hidden">
                <div className="planet-farm-content">
                  ...
                  <span>
                    {t("free")}{" "}
                    <span style={{ fontSize: "29px" }}>{t("resource")}</span>
                  </span>
                  <div
                    className={`planet-farm-tasks ${
                      resource ? "flex" : "hidden"
            ...
            <div className="planet__detail-builds">
              ...
              <div className="wrapper container without slider m-hidden max-visible">
                ...
                >
                  {items.map((item) => (
                    <SwiperSlide
                      key={item.id}
                      className="swiper-container-slide"
                    >
                      <PlanetDetailItem
                        key={item.id}
                        title={item.title}
              ...
              <img className="ten max-hidden" src="/builds/ten.png" alt="" />
            </div>
          </div>
        )}
