...
export default function Planet({ idx, planet, update }) {
  ...
  const getInitState = () => {
    setValue(
      window?.user?.wallet?.value?.find((bal) => bal.element === element.id)
        ?.value
    );
  };
  ...
  const updateFn = debounce(async (val) => {
    ...
    if (window.user?.wallet) {
      ...
      const currentElem = balance?.find((item) => item.element === element.id);
  ...
  const debounceFn = useCallback((click) => {
    updateFn(click);
  }, []);
  ...
  const userHasPlanet = () => {
    if (planet?.user_planets) {
      ...
      const idx = plData?.find(
        (item) => item?.planetId === id && item?.userId === window?.user?.id
      );
      if (idx?.id) {
        return true;
      }
    ...
    >
      <div className="animated-border" ref={animated}>
        ...
        <div className="planet__information">
          ...
          <p className="planet__lvl">
            {t("level")}{" "}
            {userHasPlanet()
              ? planet.user_planets.find((item) => item.planetId === id)?.level
              : 1}
          </p>
          <p className="planet__speed">
            {t("speed")}:{" "}
            {userHasPlanet()
              ? planet.user_planets.find((item) => item.planetId === id)
                  .level == 2
                ? 0.1
                : 0.05
              : 0.00005}{" "}
          ...
          <p className="planet__description">
            {t("extractedResource")} {element?.name}({element?.symbol})
          </p>
        ...
        <div className="planet__row">
          {userHasPlanet() ? (
            <button className="btn upgrade" onClick={updatePlanetSpeed}>
              {t("update")}
            </button>
          ) : (
            <a
          ...
          <Link to={`/planet/${id}`}>
            <button className="btn">Добывать ресурс</button>
          </Link>
          {forLaboratory ? (
            <div className="planet__time-block">
              ...
              <div className="time-block__text">{t("toniumJoin")}</div>
            </div>
          ) : (
            ""
          )}
