...
import { IAllinace, IHistory, IUser, IWallet, IWalletElement } from '../types/user.type';
...
export const getPlanets = async (range: number[], laboratory: boolean, userId: number) => {
    const instance = getAxios();
    ...
    const planets = await instance.get(
        `${url}/api/planets?range=${JSON.stringify(rang)}&filter=${JSON.stringify(filters())}&sort=${JSON.stringify([
            ['forLaboratory', 'DESC'],
        ])}&userId=${userId ?? 0}`,
    );
...
export const updateWalletElement = async (wallet: IWallet, value: IWalletElement[]) => {
    const instance = getAxios();
...
export const addPlanetToUser = async (planetId: number, resources: number) => {
    const instance = getAxios();
...
export const getAllUserPlanets = async () => {
    ...
    const planets = await instance.get<{ result: IUserPlanet[] }>(`${url}/api/userPlanets`);
...
export const addToAlliance = async (planetId: number) => {
    ...
    const isOk = await instance.post<{ result: IAllinace[] }>(`${url}/api/alliance`, {
        planetId,
    });
...
export const getAllianceByUser = async () => {
    const instance = getAxios();
    try {
        const planets = await instance.get<{ result: IAllinace[] }>(`${url}/api/alliance`);
        return planets.data.result;
    } catch (err) {
        console.log(err);
        return [];
...
export const updateMinedResource = async (id: number | string, mined: number) => {
    const instance = getAxios();
